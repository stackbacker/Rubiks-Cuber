...
#include "Baserubiks.h"

/*
 * Given a color, return its first letter.
 */

char Baserubiks::getColorLetter(COLOR color) {
    switch (color) {
        case COLOR::BLUE:
            return 'B';
        case COLOR::GREEN:
...
string Baserubiks::getMove(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return "L";
        case MOVE::LPRIME:
...
/*
 * Performing a move operation on the Cube using a Move Index.
 */
Baserubiks &Baserubiks::move(MOVE ind) {
    switch (ind){
        case MOVE::L:
            return this->l();
        case MOVE::LPRIME:
...
Baserubiks &Baserubiks::invert(MOVE ind) {
    switch (ind) {
        case MOVE::L:
            return this->lPrime();
        case MOVE::LPRIME:
...
/*
 * Perform a random shuffle of the cube and return the moves perfomed.
 */
vector<Baserubiks::MOVE> Baserubiks::randomShuffleCube(unsigned int times) {
    vector<MOVE> performed;
    srand(time(0));
    for(unsigned int i = 0; i < times; i++){
        unsigned int selectMove = (rand()%18);
        performed.push_back(static_cast<MOVE>(selectMove));
        this->move(static_cast<MOVE>(selectMove));
    }
    return performed;
}

// Helper function to get the string of the corners.

string Baserubiks::getCornerColorString(uint8_t ind) const {
    string str = "";
...
uint8_t Baserubiks::getCornerIndex(uint8_t ind) const {
...
uint8_t Baserubiks::getCornerOrientation(uint8_t ind) const {
    string corner = getCornerColorString(ind);

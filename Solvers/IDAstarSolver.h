...
#include "../CornerPatternDB/CornerPatternDB.h"
...
private:
    ...
    unordered_map<T, Baserubiks::MOVE, H> move_done;
...
//        priority_queue contains pair(Node, move done to reach that)
        ...
        while (!pq.empty()) {
            ...
            move_done[node.cube] = Baserubiks::MOVE(p.second);

            if (node.cube.isSolved()) return make_pair(node.cube, bound);
            node.depth++;
            for (int i = 0; i < 18; i++) {
                auto curr_move = Baserubiks::MOVE(i);
...
public:
    ...
    vector<Baserubiks::MOVE> solve() {
        ...
        while (!(curr_cube == rubiksCube)) {
            Baserubiks::MOVE curr_move = move_done[curr_cube];
